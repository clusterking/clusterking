...
def Icalzero(epsL, epsR, epsSR, epsSL, epsT, q2,  El):
    ...
    return mtau * np.sqrt(q2) * np.absolute(  H0val )**2  +  4*mtau*np.sqrt(q2)*np.absolute(   Hpmval  +  H0tval  )**2  + 2j*mtau**2*H0val*np.conjugate(  Hpmval  + H0tval  )    -2j*q2*np.conjugate(H0val)*( Hpmval  + H0tval  )
...
def IcalzeroI(epsL, epsR, epsSR, epsSL, epsT, q2,  El):
    ...
    return - np.sqrt(q2)*np.conjugate(H0val)*(   mtau*Htval   + np.sqrt(q2)*HSval  )  -2j*mtau*np.conjugate( Hpmval  +  H0tval )*(  mtau*Htval  + np.sqrt(q2)*HSval   )
...
def GammaI0p(epsL, epsR, epsSR, epsSL, epsT, q2,  El):
    ...
    return 2*np.real(   (        2j*np.sqrt(q2)*(  Hpmval  +H0tval  )  -mtau*H0val    )*np.conjugate(  mtau*Htval  + np.sqrt(q2)*HSval    )     )
